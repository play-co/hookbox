...
filePath:"jsio/base.js"},hookbox:{src:'jsio("from net import connect as jsioConnect");jsio("from net.protocols.rtjp import RTJPProtocol");exports.logging=logging;exports.connect=function(d,a){d.match("/$")||(d+="/");var b=new HookBoxProtocol(d,a);window.WebSocket?jsioConnect(b,"websocket",{url:d.replace("http://","ws://")+"ws"}):jsioConnect(b,"csp",{url:d+"csp"});return b};\nvar Subscription=Class(function(){this.init=function(d){this.channelName=d.channel_name;this.history=d.history;this.historySize=d.history_size;this.state=d.state;this.presence=d.presence;this.canceled=false};this.onPublish=function(){};this.onSubscribe=function(){};this.onUnsubscribe=function(){};this.onState=function(){};this.frame=function(d,a){logger.debug("received frame",d,a);switch(d){case "PUBLISH":if(this.historySize)for(this.history.push(["PUBLISH",{user:a.user,payload:a.payload}]);this.history.length>\nthis.historySize;)this.history.shift();this.onPublish(a);break;case "UNSUBSCRIBE":if(this.historySize)for(this.history.push(["UNSUBSCRIBE",{user:a.user}]);this.history.length>this.historySize;)this.history.shift();for(var b=0,c;c=this.presence[b];++b)if(c==a.user){this.presence.splice(b,1);break}this.onUnsubscribe(a);break;case "SUBSCRIBE":if(this.historySize)for(this.history.push(["SUBSCRIBE",{user:a.user}]);this.history.length>this.historySize;)this.history.shift();this.presence.push(a.user);this.onSubscribe(a);\nbreak;case "STATE_UPDATE":for(b=0;c=a.deletes[b];++b)delete this.state[c];for(c in a.updates)this.state[c]=a.updates[c];this.onState(a);break}};this.cancel=function(){if(!this.canceled){logger.debug("calling this._onCancel()");this._onCancel()}};this._onCancel=function(){}});\nHookBoxProtocol=Class([RTJPProtocol],function(d){this.onOpen=function(){};this.onClose=function(){};this.onError=function(){};this.onSubscribed=function(){};this.onUnsubscribed=function(){};this.init=function(a,b){d(this,"init",[]);this.url=a;try{this.cookieString=b||document.cookie}catch(c){this.cookieString=""}this.connected=false;this._subscriptions={};this._buffered_subs=[];this._publishes=[];this._errors={};this.username=null};this.subscribe=function(a){this.connected?this.sendFrame("SUBSCRIBE",\n{channel_name:a}):this._buffered_subs.push(a)};this.publish=function(a,b){this.connected?this.sendFrame("PUBLISH",{channel_name:a,payload:JSON.stringify(b)}):this._publishes.push([a,b])};this.connectionMade=function(){logger.debug("connectionMade");this.transport.setEncoding("utf8");this.sendFrame("CONNECT",{cookie_string:this.cookieString})};this.frameReceived=function(a,b,c){switch(b){case "CONNECTED":this.connected=true;for(this.username=c.name;this._buffered_subs.length;)this.sendFrame("SUBSCRIBE",\n{channel_name:this._buffered_subs.shift()});for(;this._publishes.length;)this.publish.apply(this,this._publishes.splice(0,1)[0]);this.onOpen();break;case "SUBSCRIBE":if(c.user==this.username){b=new Subscription(c);this._subscriptions[c.channel_name]=b;b._onCancel=bind(this,function(){this.sendFrame("UNSUBSCRIBE",{channel_name:c.channel_name})});this.onSubscribed(c.channel_name,b);K=b}else this._subscriptions[c.channel_name].frame(b,c);break;case "STATE_UPDATE":case "PUBLISH":a=this._subscriptions[c.channel_name];\na.frame(b,c);break;case "UNSUBSCRIBE":a=this._subscriptions[c.channel_name];a.canceled=true;a.frame(b,c);if(c.user==this.username){delete this._subscriptions[c.channel_name];this.onUnsubscribed(a,c)}break;case "ERROR":this.onError(c);break}};this.connectionLost=function(){logger.debug("connectionLost");this.connected=false;this.onClose()};this.reconnect=function(){jsioConnect(this,this.url)};this.disconnect=function(){this.transport.loseConnection()}});\n',
...
"undefined"&&process.version)jsio.setEnv("node");else if(typeof XMLHttpRequest!="undefined"||typeof ActiveXObject!="undefined")jsio.setEnv("browser")})();jsio("import hookbox as hookbox");
